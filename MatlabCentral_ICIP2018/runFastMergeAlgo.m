...
if getTotalPerfMultfromArea(EL,1:length(ELLSET),250) == 1
    firstSol = 1;
else
    firstSol = 0;
end
    ...
    for i=1:M,
        for j=i+1:M,
            ...
            if EL(i).Label == i && EL(j).Label == j && ConnMatrix(lab1,lab2) > 0, %check for merging
                ...
                if getTotalPerfMultfromArea(newEL,1:length(newEL),250) < 1
                    continue;
                end
...
%     
    ...
    if (RES_AICBIC < minAICBIC && getTotalPerfMultfromArea(EL,ELLSET,250) == 1) || firstSol == 0,
        firstSol = 1;
        BestIClust = IClust;
        BEST_ELLSET = ELLSET;
        minAICBIC = RES_AICBIC;
